...
class LightShader extends SpriteShader {
    ...
    _loadToShader(aCamera) {
        ...
        if (this.mLight.isLightOn()) {
            let p = this.mLight.getPosition(); // this.mCamera.wcPosToPixel(this.mLight.getPosition());
            let r = this.mLight.getRadius(); // this.mCamera.wcSizeToPixel(this.mLight.getRadius());
